...
export async function POST(request: Request) {
  ...
  const signatures = [...header.matchAll(/v1=([^,]+)/g)].map(x => x[1]);
  ...
  if (!signatures.includes(expected)) return new Response("Assinatura inválida", { status: 400 });
  ...
  if (event.type === "charge.refunded" && orderId) {
    await supabaseServer
      .from("orders")
