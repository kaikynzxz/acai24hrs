...
const products: Product[] = [
  ...
  { id: "shake-ferrero", name: "Milk-shake Ferrero Rocher", price: 22.5, oldPrice: 25, category: "Milk-shakes", description: "500ml cremoso e refrescante. Escolha sua cobertura.", milkshake: true, badge: "10% OFF", image: "https://client-assets.anota.ai/produtos/6876f462629ff00019fb2856/202604141715_LB07_iblob" },
  { id: "shake-morango", name: "Milk-shake de Morango", price: 22.5, oldPrice: 25, category: "Milk-shakes", description: "500ml cremoso e refrescante. Escolha sua cobertura.", milkshake: true, badge: "10% OFF", image: "https://client-assets.anota.ai/produtos/6876f462629ff00019fb2856/202604162102_YNUM_blob" },
  { id: "shake-baunilha", name: "Milk-shake de Baunilha", price: 22.5, oldPrice: 25, category: "Milk-shakes", description: "500ml cremoso e refrescante. Escolha sua cobertura.", milkshake: true, badge: "10% OFF", image: "https://client-assets.anota.ai/produtos/6876f462629ff00019fb2856/202605292042_6U5J_iblob" },
  { id: "shake-ovomaltine", name: "Milk-shake Ovomaltine", price: 22.5, oldPrice: 25, category: "Milk-shakes", description: "500ml cremoso e refrescante. Escolha sua cobertura.", milkshake: true, badge: "10% OFF", image: "https://client-assets.anota.ai/produtos/6876f462629ff00019fb2856/202604141716_X737_iblob" },
...
export default function Store() {
  ...
  return <main>
    ...
    {checkout && <div className="overlay"><section className="checkout" role="dialog" aria-modal="true"><button className="close" onClick={() => setCheckout(false)}>×</button><div><small>ÚLTIMO PASSO</small><h2>Entrega e pagamento</h2></div><form onSubmit={pay}><label>Nome completo<input name="name" required maxLength={120} autoComplete="name" placeholder="Seu nome" /></label><label>WhatsApp<input name="phone" required maxLength={30} inputMode="tel" autoComplete="tel" placeholder="(00) 00000-0000" /></label><label>CEP<input name="cep" required inputMode="numeric" autoComplete="postal-code" value={cep} onChange={e => lookupCep(e.target.value)} placeholder="00000-000" /></label><label>Número e complemento<input name="number" required maxLength={100} placeholder="Ex.: 123, ap. 12" /></label><label className="wide">Endereço de entrega<input name="address" required maxLength={200} autoComplete="street-address" value={address} onChange={e => setAddress(e.target.value)} placeholder="Digite o CEP para preencher automaticamente" /></label>{addressHint && <p className={`address-hint wide ${addressHint.startsWith("✓") ? "ok" : ""}`}>{addressHint}</p>}<fieldset className="wide"><legend>Como quer pagar?</legend><label><input type="radio" name="payment" defaultChecked /> <span>▣</span> Pix <small>Aprovação imediata</small></label><label><input type="radio" name="payment" /> <span>▤</span> Cartão <small>Crédito ou débito</small></label></fieldset><label className="consent wide"><input type="checkbox" name="privacyAccepted" required /><span>Li e aceito os <a href="/termos" target="_blank">Termos</a> e a <a href="/privacidade" target="_blank">Política de Privacidade</a>. Meus dados serão usados para processar e entregar o pedido.</span></label><label className="consent wide optional"><input type="checkbox" name="marketingConsent" /><span>Quero receber promoções pelo WhatsApp. Opcional e revogável a qualquer momento.</span></label>{notice && <p className="notice wide">{notice}</p>}<div className="checkout-total wide"><span>Total do pedido</span><b>{money(total)}</b></div><button className="primary wide" type="submit">Pagar com segurança <span>→</span></button></form><small className="secure">🔒 O pagamento é processado pelo provedor financeiro. A loja não armazena os dados completos do cartão.</small></section></div>}
